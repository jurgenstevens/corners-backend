...
function checkAdmin(req, res, next) {
  if (req.user?.authorizationLevel === AUTH_LEVELS.ADMIN) return next()
  return res.status(403).json({ err: 'Admins only' })
}

function checkBusiness(req, res, next) {
  if (req.user?.authorizationLevel === AUTH_LEVELS.BUSINESS) return next()
  return res.status(403).json({ err: 'Business only' })
}

function checkDistributor(req, res, next) {
  if (req.user?.authorizationLevel === AUTH_LEVELS.DISTRIBUTOR) return next()
  return res.status(403).json({ err: 'Distributors only' })
}

function checkPatron(req, res, next) {
  if (req.user?.authorizationLevel === AUTH_LEVELS.PATRON) return next()
...
const decodeUserFromToken = (req, res, next) => {
  ...
  jwt.verify(token, SECRET, (err, decoded) => {
    ...
    req.user = decoded  // was decoded.user — bug fixed
    next()
  })
}
